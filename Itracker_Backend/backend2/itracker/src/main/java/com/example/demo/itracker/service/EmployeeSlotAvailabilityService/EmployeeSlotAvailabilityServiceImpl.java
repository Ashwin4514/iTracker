...
import com.example.demo.itracker.repository.EmployeeRepository;
import com.example.demo.itracker.repository.EmployeeSlotAvailabilityRepository;
import com.example.demo.itracker.utility.EmployeeSlotDetailsUtility;
import com.example.demo.itracker.utility.RequestBodyEmployeeSlotAvailability;
import com.example.demo.itracker.utility.Response;
...
public class EmployeeSlotAvailabilityServiceImpl implements EmployeeSlotAvailabilityService {
	...
	public Response addSlot(String id, RequestBodyEmployeeSlotAvailability rq) {
		Response response = new Response();
		try {
//			if(esarepo.getEmployeeSlotAvailabilityByDateAndEmployeeId(rq.getDate(), id)!=null) {
//				throw new Exception("You already mentioned your availability on this date :"+rq.getDate());
//			}
			EmployeeSlotAvailability obj = new EmployeeSlotAvailability();
			obj.setId(rq.getId());
			obj.setDate(rq.getDate());
	...
	@Override
	public List<EmployeeSlotDetailsUtility> getALLSlotsDetails() {
		// TODO Auto-generated method stub
		List<Employee> employees = erepo.findAll();
		List<EmployeeSlotDetailsUtility> employeeList=new LinkedList<>();
		for (Employee employee : employees) {
			for(EmployeeSlotAvailability esa:employee.getSlotsAvailable()) {
				EmployeeSlotDetailsUtility employeeSlotDetailsUtility=new EmployeeSlotDetailsUtility();
				employeeSlotDetailsUtility.setEmployeeId(employee.getEmployeeId());
				employeeSlotDetailsUtility.setName(employee.getName());
				employeeSlotDetailsUtility.setEmail(employee.getEmail());
				employeeSlotDetailsUtility.setDate(esa.getDate());
				employeeSlotDetailsUtility.setTime(esa.getTime());
				employeeList.add(employeeSlotDetailsUtility);
			}
		}
		return employeeList;
	}
